...
type
  ...
  TFrmItac = class(TForm)
    ...
    procedure SerialNrforAttriClick(Sender: TObject);
  ...
  private
    ...
    procedure AddLog_itac(const Param: String);
    function LoadingStation(sSerial : String) : Boolean;
  public
    { Public declarations }
    bApiInitialize : Boolean;
    LastMessage : String;
    FAddLog : procedure (Param : String) of object;
    ...
    function  GetAttributeforSerial(sSerial, sAttributeCode : String; var sAttributeValue : String ) : Integer;
    ...
    function  CalibrationState(sStationNr, sSerial : String; var LoopCount : Integer) : Integer;
    ...
    function getCurrentCal(): Integer;
...
uses GenUtils, Math, FormMain, FormLog;
...
begin
    ...
    try
        ...
        AddLog_itac('apiShutdown : ' + IntToStr(Ret) + ' : ' + StrPas(errorString));
    finally
        apiFreeBuffers;
    end;
end;
    ...
    StationNo := Edit_StationNo.Text;
{
    ...
    SessionID := apiLogin(seccValData, errorString);
    AddLog_itac('apiLogin : SessionID=' + IntToStr(SessionID) + ' ' + StrPas(errorString));
...
begin
    ...
    SessionID := -1;
    AddLog_itac('apiLogout : ' + StrPas(errorString));

    apiFreeBuffers;
end;

function TFrmItac.getCurrentCal(): Integer;
var
    // In
    stationNr    : APISTRING;
...
begin
    Result := -9999;
    if not bApiInitialize then exit;
//function getCurrentCalendar( sessionId : APILONG; stationNr : APISTRING;
// var shift : APILONG; var calendarData: TCalendarData; var errorString : APISTRING) : APILONG; cdecl;

    stationNr       := AllocMem(MAX_PARAMETER_STRING_LENGTH * sizeof(APICHAR));
    try
        helperScanf(stationNr,       Edit_StationNo.Text);

        Result :=  ewMII.getCurrentCalendar(SessionID, stationNr, shift, @calendarData, errorString);
        LastMessage := Format('getCurrentCalendar : (%d) %s', [ Result, StrPas(errorString)]);
        AddLog(LastMessage);

        with SystemTime do
        begin
            wYear         := calendarData.year;
            wMonth        := calendarData.month;
            wDay          := calendarData.day;
            wHour         := calendarData.hour;
            wMinute       := calendarData.minute;
            wSecond       := calendarData.second;
            wMilliseconds := 0;
        end;
...
//    iTacINI.LoadData;
    ...
    //InitItac;
   if not Cb_DisableITAC.Checked then begin
        BtnApiInitClick(Sender);
        if bApiInitialize then Login;
    end;  
end;

procedure TFrmItac.InitItac;
begin
    if not Cb_DisableITAC.Checked then begin
        BtnApiInitClick(nil);
        if bApiInitialize then Login;
...
procedure TFrmItac.AddLog_itac(const Param : String);
begin
    ...
    if Assigned(FAddLog) then FAddLog(Param);
...
begin
    ...
    try
        ...
        AddLog_itac(Format('getAttributesForWorkorder : (%d) %s', [ Result, StrPas(errorString)]));
...
function TFrmItac.CalibrationState(sStationNr, sSerial : String; var LoopCount : Integer) : Integer;
var Str, sAttributeCode, sAttributeValue : String;
    // In
    StationNr, serialnumber, serialNumberPos : APISTRING;
    ProcessLayer : Integer;
...
begin
    ...
    try
        helperScanf(StationNr,       sStationNr);
        helperScanf(SerialNumber,    sSerial);
        helperScanf(SerialNumberPos, '-1');
...
begin
//    if not Cb_DisableITAC.Checked then
    BtnLogOutClick(nil);
...
begin
    ...
    AddLog_itac(Format('LOT = %s, SN = %s', [SensorLot, SensorSN]));
end;
//------------------------------------------------------------------------------
...
var Ret : APILONG;
    ...
    sIdx, sName, sValue, sMin, sMax, sNom, sUnit, sFail : String;
    sString : String;
begin
    Result := -9999;
    ...
    try
        ...
        begin
            ...
            if (sMax = '') then begin
                ...
                helperScanf(resultData[nIdx].max,     '0');  //2015-11-25 itac max 는0으로
            ...
            if IsFloat(sMin) and IsFloat(sMax) and IsFloat(sValue) then
...
begin
    ...
    begin
        ...
        resultData^.failCode := StrToIntDef(sResult, 0);
    end;
...
begin
    ...
    try
        ...
        AddLog_itac(Format('verifyMerge : %s (%d) %s', [sSerial, Ret, errorString]));
        Result := (Ret = 0);
    finally
        FreeMem(stationNr);
    ...
    try
        ...
        begin
            for i:= 0 to serialNumberArraySize-1 do begin
                ...
                AddLog_itac(Format('(%d), SerialNo=%s, serialNumberPos=%s',
...
function TFrmItac.GetAttributeforSerial(sSerial, sAttributeCode: String; var sAttributeValue: String): Integer;
var
    stationNr: APISTRING;
    attributeCode: APISTRING;
    attributeValue: APISTRING;
    serialNrArraySize : APILONG;
    serialNrArray : PSnrAttributeInfoArray;//TSnrAttributeInfo;//PSerialNumberStructArray;
    errorString: APISTRING;
    ArrayIdx : PSnrAttributeInfoArray;//TSnrAttributeInfo;//PSerialNumberStructArray;
    sSerialNo : String;
    i : integer;
begin
    stationNr   := AllocMem(MAX_PARAMETER_STRING_LENGTH * sizeof(APICHAR));
    ...
    try
        helperScanf(StationNr,  Edit_StationNo.Text);
        helperScanf(attributeCode,  sAttributeCode);
        helperScanf(attributeValue,  sSerial);

        serialNrArraySize := 1;

        Result := ewMII.getAttributesForSerialNumber(
                sessionId,
                stationNr,
                attributeValue,
                attributeCode,
        ...
        LastMessage := Format('getSerialNumbersForAttribute : (%d) %s', [ Result, errorString]);
        AddLog(LastMessage);

        if (Result = 0) then
        begin
            for i:= 0 to serialNrArraySize-1 do begin
                ArrayIdx := serialNrArray;
                AddLog(Format('(%d), SerialNo=%s',
                    [i+1, ArrayIdx^.attributeValue]));

                sAttributeValue := StrPas(ArrayIdx^.attributeValue);
            end;
        end;
    finally
        FreeMem(stationNr);
...
procedure TFrmItac.SerialNrforAttriClick(Sender: TObject);
var sAttribute: String;
begin
    GetAttributeforSerial(Edit_SerialNo.Text, Combo_AttributeCode.Text, sAttribute);
end;



procedure TFrmItac.Button2Click(Sender: TObject);
VAR SSTRING : STRING;
begin
    GetAttributeforSerial(frmmain.lb_Barcode.Caption,'HMF',SSTRING);
end;
