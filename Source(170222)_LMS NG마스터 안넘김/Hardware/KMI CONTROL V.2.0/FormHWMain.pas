...
type
  ...
  public
    ...
    function ReadDO(nBDNo, nChNo: Integer): Boolean;
...
uses GenUtils, FormIOScale, FormADBuff{, FormHwTest,  FormCanDiag},
  FormMain;  // FormComm
...
//------------------------------------------------------------------------------
// ReadDO
function TFrmHWMain.ReadDO(nBDNo, nChNo: Integer): Boolean;
var nResNo, nStation, nSlot : Integer;
begin
    Result := False;
    if (nBDNo > DOCnt) or (nBDNo < 1) or (nChNo < 1) or (nChNo > MAX_CHANNEL) then exit;
...
begin
    ...
    if (Cb_AutoConnect.Checked) and (BtnDisconnect.Tag = 0) and (not frmMain.gbRunning) then
    begin
