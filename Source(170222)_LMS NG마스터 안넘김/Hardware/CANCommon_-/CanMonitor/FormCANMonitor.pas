...
type
  TFrmCANMonitor = class(TForm)
    ...
    procedure TreeViewCustomDrawItem(Sender: TCustomTreeView;
      Node: TTreeNode; State: TCustomDrawState; var DefaultDraw: Boolean);
    procedure BtnTxClearClick(Sender: TObject);
    ...
    procedure BtnRxClearClick(Sender: TObject);
  private
  ...
  public
    ...
    procedure UpdateSignal(MessageNode : TTreeNode; DBCMessage : TCAN_Message; CanMsg : TCanMessage);
...
procedure TFrmCANMonitor.UpdateSignal(MessageNode : TTreeNode; DBCMessage : TCAN_Message; CanMsg : TCanMessage);
var j : Integer;
    SignalNode : TTreeNode;
    CanSignal  : TCAN_Signal;
    Str, sValue, sTblValue : String;
begin
    if (MessageNode = nil) or (DBCMessage = nil) then exit;
    ...
    begin
        ...
        if (SignalNode <> nil) then begin
            ...
            // 테이블에서 값을 얻는다.
            sValue := FormatFloat('0.###', CanSignal.GetValue(CanMsg) );
    ...
    try
        ...
        begin
            ...
            begin
                ...
                if MessageNode.Expanded then
                begin
                    // Message정보를 모두 Update한다.
                    UpdateSignal(MessageNode, DBCMessage, PCanMsg^);
                 {
                    SignalNode := MessageNode.getFirstChild;
...
procedure TFrmCANMonitor.TreeViewExpanded(Sender: TObject;
  Node: TTreeNode);
var Str : String;
    DBCMessage : TCAN_Message;
    CanMsg : TCanMessage;
begin
    Str := Node.Text;
    // Str = '000=00 00 00 00     :  100  12345.0 [Message Name]  이런형식...

    CanMsg := StrToCanMsg(Str);
    DBCMessage := m_CanDB.FindCanMessage( CanMsg.CANId );
    UpdateSignal(Node, DBCMessage, CanMsg);
end;
...
begin
    ...
    begin
        ...
        Str := Caption;
        Caption := GetToken(Str, ':') + ':' + ExtractFileName(FileName);
    end;
end;
...
begin
    ...
    if (PageControl.ActivePageIndex = 2) and (Cb_View.Checked) then
    begin
        // Message Log를 기록한다.
        CANUNit.Lock;
...
procedure TFrmCANMonitor.BtnTxClearClick(Sender: TObject);
begin
    // 보내는 메세지를 초기화 한다.
    CanUnit.Clear(CANPort, True, False);
end;

procedure TFrmCANMonitor.BtnRxClearClick(Sender: TObject);
begin
    // 받은 메세지를 초기화 한다.
    TreeView.Items.Clear;
...
var i : Integer;
    ...
    TreeNode : TTreeNode;
begin
    SignalName := InputBox('Find Signal', 'SIGNAL 명을 입력하세요',  '');
    ...
    for i:= 0 to TreeViewCanDB.Items.Count-1 do begin
        ...
        RdStr := TreeNode.Text;
       // Str := GetTokenTrim(RdStr, ':');
        if Pos(SignalName, RdStr) > 0 then begin
            TreeViewCanDB.Selected := TreeNode;
            if MessageBox(Format('[%s]명칭이 존재하는 SIGNAL을 찾았습니다. 추가검색할까요?', [SignalName]), 'SIGNAL 확인', MB_YESNO) <> ID_YES then
                break;
        end;
    end;

    if (TreeViewCanDB.Selected = nil) then
        MessageBox('검색한 Signal명이 없습니다.', '확인', MB_OK or MB_ICONWARNING);
end;
...
procedure TFrmCANMonitor.SpeedButton1Click(Sender: TObject);
begin
    ListBoxLog.Clear;
end;


procedure TFrmCANMonitor.Panel2Click(Sender: TObject);
begin
