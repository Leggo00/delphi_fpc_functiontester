...
type
  TFrmKMIMBCOPWM = class(TFrmDevice)
    Panel1: TPanel;
    ...
    TabSheet2: TTabSheet;
    Grid: TAdvStringGrid;
    btn_MBCO_Set: TButton;
    btn_MBCO_RESET: TButton;
    btn_MBCO_PosSet: TButton;
    edt_MBCO_POS_dec: TNumberEdit;
    ...
    procedure btn_MBCO_SetClick(Sender: TObject);
    procedure btn_MBCO_RESETClick(Sender: TObject);
    procedure btn_MBCO_PosSetClick(Sender: TObject);
  private
...
function TFrmKMIMBCOPWM.DevClose(Param: String): Integer;
begin
    KMIComm.CommClose;
    StatusBar.Panels[0].Text := 'DISCONNECT';
end;

function TFrmKMIMBCOPWM.DevInit(Param: String): Integer;
begin
//
end;

function TFrmKMIMBCOPWM.DevOpen(Param: String): Integer;
begin
...
function TFrmKMIMBCOPWM.DevRead(Param: String): String;
begin
//
end;

function TFrmKMIMBCOPWM.DevFileOpen(Param: String): boolean;
begin
    if LoadTheta(Param) then Result := True
                        else Result := False;
end;

function TFrmKMIMBCOPWM.DevSend(Param: String): Integer;
...
begin
    ...
    if (Cmd = 'MBCO') then
    begin
        edt_MBCO_POS_dec.Text := Option;
        btn_MBCO_PosSetClick(Self);
    end else
    if (Cmd = 'MBCO_0') then
    begin
        Edit_Ch1Freq.Text := GetToken(Option, ',');
    ...
    if (Cmd = 'MBCORESET') then
    begin
        btn_MBCO_RESETClick(Self);
    end else
    if (Cmd = 'MBCOSET') then
    begin
        btn_MBCO_SetClick(Self);
    end else
...
procedure TFrmKMIMBCOPWM.FormCreate(Sender: TObject);
begin
    INI.LoadData;
...
procedure TFrmKMIMBCOPWM.FormDestroy(Sender: TObject);
begin
    KMIComm.Free;
end;

procedure TFrmKMIMBCOPWM.BtnConnectClick(Sender: TObject);
begin
    DevOpen('');
end;

procedure TFrmKMIMBCOPWM.FormClose(Sender: TObject; var Action: TCloseAction);
begin
    //SaveTheta;
    INI.SaveData;
end;

procedure TFrmKMIMBCOPWM.SpeedButton2Click(Sender: TObject);
begin
    DevClose('');
end;

procedure TFrmKMIMBCOPWM.BtnResetClick(Sender: TObject);
var Str : String;
begin
    // Reset.
    KMIComm.CommSend($03, []);
...
procedure TFrmKMIMBCOPWM.SetPWM(Sender: TObject);
...
procedure TFrmKMIMBCOPWM.AddLog(Param: String);
begin
    with Memo.Lines do begin
...
procedure TFrmKMIMBCOPWM.SignalResetClick(Sender: TObject);
var fTime : Double;
begin
    fTime := GetTickCountEX;
...
procedure TFrmKMIMBCOPWM.BtnWriteClick(Sender: TObject);
var nData : Byte;
    fTime : Double;
begin
    nData := 0;
...
procedure TFrmKMIMBCOPWM.BtnReadClick(Sender: TObject);
var nData : Byte;
    fTime : Double;
begin
    fTime := GetTickCountEX;
...
procedure TFrmKMIMBCOPWM.SpeedButton9Click(Sender: TObject);
var nData : Byte;
    fTime : Double;
begin
    fTime := GetTickCountEX;
...
procedure TFrmKMIMBCOPWM.SpeedButton10Click(Sender: TObject);
var nData : Byte;
    nInterval : Word;
    fTime : Double;
...
procedure TFrmKMIMBCOPWM.MakeGrid;
...
procedure TFrmKMIMBCOPWM.SetGrid(nCol, nRow, nStartTheta, nStartDegree, nIncDegreeCount: integer);
var
    nDeg : integer;
    nStartIdx : integer;
begin
...
Function TFrmKMIMBCOPWM.LoadTheta(sFileName : String): boolean;
...
procedure TFrmKMIMBCOPWM.GridGetAlignment(Sender: TObject; ARow, ACol: Integer;
  var HAlign: TAlignment; var VAlign: TVAlignment);
begin
    HAlign := taCenter;
end;

procedure TFrmKMIMBCOPWM.GridGetCellColor(Sender: TObject; ARow, ACol: Integer;
  AState: TGridDrawState; ABrush: TBrush; AFont: TFont);
var nDegree : Integer;
begin
...
procedure TFrmKMIMBCOPWM.btn_MBCO_SetClick(Sender: TObject);
var nData : Byte;
    fTime : Double;
begin
    fTime := GetTickCountEX;
    nData := $01;
    KMIComm.CommSend($30, [nData]);
...
procedure TFrmKMIMBCOPWM.btn_MBCO_RESETClick(Sender: TObject);
var nData : Byte;
    fTime : Double;
begin
    fTime := GetTickCountEX;
...
procedure TFrmKMIMBCOPWM.btn_MBCO_PosSetClick(Sender: TObject);
var
    nData : Integer;
    nHex1, nHex2, sStr : string;
    fTime : Double;
    ...
    procedure MakeBCD( StartIdx, nValue1{, nValue2} : Integer);
    begin
        sStr := IntToHex(nValue1, 4);
        nHex1 := copy(sStr, 1, 2);
        nHex2 := copy(sStr, 3, 2);

        Buff[0] := HexToIntDef(nHex1, 0);
        Buff[1] := HexToIntDef(nhex2, 0);
    end;
begin
    fTime := GetTickCountEX;
    nData := StrToIntDef(edt_MBCO_POS_dec.Text, 0);

    SetLength(Buff, 2);
    MakeBCD(0, Round(nData));
    //Buff :=
    KMIComm.CommSend($31, Buff);
