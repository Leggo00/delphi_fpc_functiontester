...
type
  ...
  TFrmItac = class(TForm)
    ...
    TimerConnect: TTimer;
    CheckRoomTester: ZIniCheckBox;
    SerialNrforAttri: TSpeedButton;
    Label10: TLabel;
    Edt_YMD: ZIniEdit;
...
begin
    ...
    try
        ...
        begin
            ...
            if (sMax = '') then begin
                helperScanf(resultData[nIdx].min,     '0');
                helperScanf(resultData[nIdx].max,     '0');
                //helperScanf(resultData[nIdx].nom,     sMin);
                // 2013-01-31 by shs = because of uploaddata server
...
begin
    ...
    sItacPartNo := SnrInfoData.partNr;
    // 2) Station No를 가져온다
...
var
    ...
    serialNrArray : PSerialNumberStructArray;
    errorString: APISTRING;
    ArrayIdx : PSerialNumberStructArray;
    sSerialNo : String;
    i : integer;
begin
    stationNr   := AllocMem(MAX_PARAMETER_STRING_LENGTH * sizeof(APICHAR));
    ...
    try
        ...
        Result := ewMII.getSerialNumbersForAttribute(
                sessionId,
                stationNr,
                attributeCode,
                attributeValue,
        ...
        begin
            for i:= 0 to serialNrArraySize-1 do begin
                ...
                AddLog(Format('(%d), SerialNo=%s',
                    [i+1, ArrayIdx^.serialNumber]));

                sAttributeValue := StrPas(ArrayIdx^.serialNumber);
            end;
        end;
    finally
        FreeMem(stationNr);
