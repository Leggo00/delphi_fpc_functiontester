...
//------------------------------------------------------------------------------
function TFrmXantrex.DevOpen(Param: String): Integer;
begin
    Comm.Disconnect;
...
//------------------------------------------------------------------------------
function TFrmXantrex.DevClose(Param: String): Integer;
begin
    Comm.Disconnect;
    StatusBar.SimpleText := 'DISCONNECT';
end;
//------------------------------------------------------------------------------
function TFrmXantrex.DevInit(Param: String): Integer;
begin
    Comm.FlushBuffers(True, True);
    DevSend('CLS');
end;
//------------------------------------------------------------------------------
function TFrmXantrex.DevRead(Param: String): String;
var fValue : Double;
begin
    fValue := Getcurrent();
...
//------------------------------------------------------------------------------
function TFrmXantrex.DevSend(Param: String): Integer;
begin
    bWaitComm := True;
    if Comm.Connected then
    begin
...
procedure TFrmXantrex.FormCreate(Sender: TObject);
begin
    INI.LoadData;
    DevOpen('');
end;

procedure TFrmXantrex.FormDestroy(Sender: TObject);
begin
    INI.SaveData;
end;

procedure TFrmXantrex.BtnOnClick(Sender: TObject);
begin
    SetPower( StrToFloatDef(inputedit.Text, 0), StrToFloatDef(inputCURR.Text, 0), True);
end;

function TFrmXantrex.SetPower( fVolt, fCurr : double; bONOFF : Boolean) : Boolean;
begin
    MemoRX.Clear;
...
procedure TFrmXantrex.SpeedButton1Click(Sender: TObject);
begin
    if not Comm.Connected then DevOpen('')
                          else DevClose('');
end;

procedure TFrmXantrex.CommReceiveData(Sender: TObject; DataPtr: Pointer;
  DataSize: Cardinal);
begin
...
procedure TFrmXantrex.BtnOffClick(Sender: TObject);
begin
    SetPower( StrToFloatDef(inputedit.Text, 0), StrToFloatDef(inputCURR.Text, 0), False);
end;

function TFrmXantrex.Getcurrent : Double;
var fTime : Double;
    sValue : string;
    i : Integer;
begin
    ...
    GetToken(RxStr, ' ');
    Result := StrToFloatDef(RxStr, -9999);
...
procedure TFrmXantrex.BtnLocalClick(Sender: TObject);
begin
  //  DevSend('RMT 1');
end;

procedure TFrmXantrex.BitBtn3Click(Sender: TObject);
var fValue : Double;
begin
    fValue := Getcurrent();
...
procedure TFrmXantrex.BitBtn2Click(Sender: TObject);
begin
    DevSend(Edt_Send.Text);
end;

procedure TFrmXantrex.BitinitsendClick(Sender: TObject);
begin
