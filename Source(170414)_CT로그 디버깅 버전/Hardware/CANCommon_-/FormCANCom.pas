...
begin
    ...
    begin
        ...
        AddLog(Format('> Open CAN(%d) %s KBps OK. (%f ms)', [nPort, FormatFloat('0.###',fBaudrate), (GetTickCountEX - fTime)]));
    end else
    begin
...
begin
    OnCANReadEvent := nil;
{
    INI.IniFileName := GetAppDir('') + 'CONFIG.INI';
    INI.IniSection  := Format('CAN%d', [tag]);
    INI.LoadData;
    ...
    //BtnOpenDeviceClick(Sender);   // <- 생성시에 OpenDevice제거... 별도 수행해야함.
