...
begin
    if (giErrorCode <> 0) or (sDeviceName = '') then exit;
...
var
   ...
   sDeviceType, sDeviceName, sValue: string;
begin
    TimerRead.Enabled := False;
    ...
    begin
        ...
        sDeviceType := sDeviceName[1];  // L or B
        sDeviceName := Copy(sDeviceName, 2, Length(sDeviceName)-1);

        // L은 10진수로 B는 16진수로 Address가 진행된다.
        if (sDeviceType = 'L') then
            nDevice := StrToIntDef(sDeviceName, 0)
        else
        if (sDeviceType = 'B') then
            nDevice := HexToIntDef(sDeviceName, 0);
        ...
        begin
             ...
             begin
                 ...
                 begin // Bit
                     ...
                     gDeviceList.Values[ Format('%s%s', [ sDeviceType, sDeviceName ]) ] := sValue[17-j];
                     Inc(nDevice);
                 end;
             end;
    ...
    if Visible then
        memoDeviceList.Lines.Assign(gDeviceList);
